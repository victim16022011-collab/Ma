...
-- Cập nhật MỚI NHẤT: AUTO FARM V2.5 (Delay rơi 0.75s, Sửa vạch 0.75s, Xóa đông cứng)
-- ==================================================
...
local PopupSound = Instance.new("Sound")PopupSound.SoundId = "rbxassetid://6895079853"
...
StatusLabel.Position = UDim2.new(0, 0, 0.35, 0)StatusLabel.Size = UDim2.new(1, 0, 0, 30)
...
getgenv().AutoHop_V1 = truegetgenv().AutoHop_V2 = false
...
if data.LanguageIndex ~= nil then getgenv().LanguageIndex = data.LanguageIndex endend
end)
end
end
LoadSettings()
...
SettingsFrame.AnchorPoint = Vector2.new(0.5, 0.5)SettingsFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
...
CloseButton.Text = "X"CloseButton.TextColor3 = Color3.fromRGB(255, 100, 100)
...
endlocal function CreateSectionHeader(id, text)
...
RowFrame.Size = UDim2.new(0.92, 0, 0, 75)RowFrame.ZIndex = 11
...
Knob.Parent = SliderBgKnob.AnchorPoint = Vector2.new(0, 0.5)
...
return {ForceUpdate = function(state) currentState = state; UpdateVisuals(true) end}end
...
Desc.TextSize = 13
Desc.TextXAlignment = Enum.TextXAlignment.Left
Desc.ZIndex = 12local ControlBg = Instance.new("Frame")
...
MinusBtn.MouseButton1Click:Connect(function()PlaySound(HoverSound)
...
Title.Font = Enum.Font.GothamBold
Title.Text = titleText
Title.TextColor3 = Color3.fromRGB(255, 255, 255)Title.TextSize = 20
Title.TextXAlignment = Enum.TextXAlignment.Left
Title.ZIndex = 12
Desc.Parent = RowFrame
Desc.BackgroundTransparency = 1
...
local Desc = Instance.new("TextLabel")
RowFrame.Name = "Row_" .. id
RowFrame.Parent = ScrollingFrameRowFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
RowFrame.BackgroundTransparency = 0.4
RowFrame.Size = UDim2.new(0.92, 0, 0, 75)
RowFrame.ZIndex = 11
RowCorner.CornerRadius = UDim.new(0, 12)
...
IStroke.Color = Color3.fromRGB(100, 100, 150)IStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
if state then
getgenv().AutoFarm_V1 = falseSliderFarmV1.ForceUpdate(false)
end
end)
...
getgenv().AutoFarm_Killer_V1 = stateif state then
getgenv().AutoFarm_Killer_V2 = false
...
T2 = "Auto Farm Gen V2.5", D2 = "Hit & Run: Repair all gens bar by bar.",T3 = "Auto Evade V1 (Safe)", D3 = "Killer within 20m -> Hide for 6s.",
...
UpdateRowText("2", t.T2, t.D2)UpdateRowText("3", t.T3, t.D3)
...
StatusLabel.Text = "Status: " .. CurrentStatuspcall(function()
...
local lvl = v.Valueif lvl > 0 and lvl < 100 then
...
if not char then return false end
local killersFolder = Workspace:FindFirstChild("Players") and Workspace.Players:FindFirstChild("Killers")if killersFolder and killersFolder:FindFirstChild(char.Name) then return true end
return false
end
...
local pos = nil
if obj:IsA("BasePart") thenpos = obj.Position
...
local safeGen = GetSafeGenerator(killerPos)
if safeGen then
bestSpot = safeGen:GetPivot().Position
else
bestSpot = killerPos + Vector3.new(math.random(-150, 150), 10, math.random(-150, 150))
end
end
return bestSpot
end
...
local function GetNextGenerator_V1()if not Workspace:FindFirstChild("Map") then return nil, false, false end
local ingame = Workspace.Map:FindFirstChild("Ingame")
...
for _, obj in ipairs(GameMap:GetDescendants()) do
if obj.Name == "Generator" and obj:IsA("Model") then
local progress = GetProgress(obj)if progress ~= nil then
TotalGens = TotalGens + 1
...
if IsHopping then return endIsHopping = true
...
SetStatus("Trang này đầy! Lật trang...")task.wait(0.5)
...
table.clear(IgnoreList)table.clear(HitCycle)
end
else
...
local n = string.lower(v.Name)if string.find(n, "slash") or string.find(n, "punch") or string.find(n, "carving") or string.find(n, "stab") or string.find(n, "lacerate") then
...
firesignal(v.MouseButton1Down)
firesignal(v.MouseButton1Click)
SkillCooldownTracker[n] = currentTimeend
end
end
end
end
...
if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") thenLocalPlayer.Character.Humanoid.Health = 0
end
...
local safeTargetPos = targetPos or pivot.Position
local dropPos = safeTargetPos + Vector3.new(0, dropHeight, 0)
...
if IsSpotBlocked(dropPos) then break end
...
root.Anchored = false
if getgenv().AutoFarm_V2 then
task.wait(0.75) -- [AUTO FARM V2.5 FREE]: Rơi xuống 0.75s rồi sửa máy
else
task.wait(1)
root.Anchored = true
end
...
local oldGenPos = safeTargetPos
repeat
if not getgenv().AutoFarm then break end
local curHum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
if curHum and curHum.Health <= 0 then SmartServerHop(); return end
local currentKiller = GetKiller()
if not currentKiller or not currentKiller:FindFirstChild("HumanoidRootPart") then break endif not isSurvivorModel(LocalPlayer.Character) then IsInMatch = false; break end
...
Notify("Lỗi: Không tìm thấy máy an toàn! Teleport ngẫu nhiên...")
root.Anchored = false
task.wait(0.1)
root.CFrame = CFrame.new(root.Position + Vector3.new(math.random(-100, 100), 10, math.random(-100, 100)))
root.Velocity = Vector3.zero
root.Anchored = true
task.wait(6)
root.Anchored = false
...
if not getgenv().AutoFarm then break end
local curHum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
if curHum and curHum.Health <= 0 then SmartServerHop(); return endif LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
...
Notify("Lỗi: Không tìm thấy điểm spawn! Teleport ngẫu nhiên...")
root.Anchored = false
task.wait(0.1)
root.CFrame = CFrame.new(root.Position + Vector3.new(math.random(-150, 150), 10, math.random(-150, 150)))
root.Velocity = Vector3.zero
root.Anchored = true
task.wait(3)
root.Anchored = false
...
if prompt then pcall(function() prompt:InputHoldEnd() end) endHitCycle[gen] = true
LastGenerator = gen
break
end
end
...
elselocal songData = getgenv().SongList[getgenv().CurrentSongIndex]
