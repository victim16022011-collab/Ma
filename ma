    ...
    if readfile and isfile and isfile(SaveFileName) then
        pcall(function()
            ...
            if data then
                ...
                if data.AutoEvade_V3 ~= nil then getgenv().AutoEvade_V3 = data.AutoEvade_V3 end
        ...
        if getgenv().AutoSave then
            pcall(function()
                local currentData = HttpService:JSONEncode({
                    ...
                    AutoEvade_V3 = getgenv().AutoEvade_V3,
...
local SliderFarmV1, SliderFarmV2, SliderEvadeV1, SliderEvadeV2, SliderEvadeV3, SliderKillerV1, SliderKillerV2, SliderFarmLevelV1
...
SliderEvadeV1 = CreateCyberpunkSettingRow("3", "Auto Né V1 (Cẩn Thận)", "Killer vào 20m -> Lết trốn 6s an toàn.", getgenv().AutoEvade_V1, function(state)
    ...
    if state then
        ...
        SliderEvadeV3.ForceUpdate(false)
    end
end)
...
SliderEvadeV3 = CreateCyberpunkSettingRow("5", "Auto Né V3 (Bay Lên Trời)", "Killer tới -> Bay lên máy cao 30m trốn.", getgenv().AutoEvade_V3, function(state)
    getgenv().AutoEvade_V3 = state
    if state then
        getgenv().AutoEvade_V1 = false
        getgenv().AutoEvade_V2 = false
        SliderEvadeV1.ForceUpdate(false)
...
CreateCyberpunkSettingRow("6", "Bật/Tắt Nhạc Nền", "Nghe nhạc cực chill.", getgenv().MusicEnabled, function(state)
    getgenv().MusicEnabled = state
    if getgenv().ApplyMusicSettings then getgenv().ApplyMusicSettings() end
end)

CreateCyberpunkVolumeRow("7", "Âm Lượng Nhạc", "Điều chỉnh to nhỏ cho phù hợp.", getgenv().MusicVolumePercent, function(val)
    getgenv().MusicVolumePercent = val
    if getgenv().ApplyMusicSettings then getgenv().ApplyMusicSettings() end
end)

CreateCyberpunkCycleRow("8", "Chọn Bài Hát", "Đổi bài hát yêu thích của bạn.", getgenv().SongList, getgenv().CurrentSongIndex, function(index)
    getgenv().CurrentSongIndex = index
    getgenv().IsUsingCustomMusic = false
    if getgenv().ApplyMusicSettings then getgenv().ApplyMusicSettings() end
end)

CreateCyberpunkInputRow("9", "Nhạc Custom (ID)", "Dán ID nhạc và nhấn nút ▶ để phát đè lên nhạc có sẵn.", "Nhập ID...", function(text)
...
SliderKillerV1 = CreateCyberpunkSettingRow("10", "Auto Farm Killer V1", "Đợi 3s tàng hình, spam chiêu cơ bản, quét sạch.", getgenv().AutoFarm_Killer_V1, function(state)
...
SliderKillerV2 = CreateCyberpunkSettingRow("11", "Auto Farm Killer V2", "Chờ 3s tàng hình, xả trọn bộ Skill VIP, quét sạch.", getgenv().AutoFarm_Killer_V2, function(state)
...
SliderFarmLevelV1 = CreateCyberpunkSettingRow("12", "Auto Farm Level V1", "Tự đổi tướng < Lv100 khi con đang xài đã max.", getgenv().AutoFarm_Level_V1, function(state)
    getgenv().AutoFarm_Level_V1 = state
end)

CreateSectionHeader("Sec6", "VI: Auto Server Hop")

SliderHopV1 = CreateCyberpunkSettingRow("13", "Auto Hop V1 (Mặc Định)", "Tìm server ít người, ping ổn định (<150ms).", getgenv().AutoHop_V1, function(state)
...
SliderHopV2 = CreateCyberpunkSettingRow("14", "Auto Hop V2 (Siêu Mượt)", "Tìm server ít người và ping thấp khắt khe (<120ms).", getgenv().AutoHop_V2, function(state)
...
SliderHopTime = CreateCyberpunkSettingRow("15", "Hop Sau 10 Phút", "Tự động đổi Server sau mỗi 10 phút chơi.", getgenv().AutoHop_TimeLimit, function(state)
    getgenv().AutoHop_TimeLimit = state
end)

CreateSectionHeader("Sec7", "VII: Setting")

SliderFixLag = CreateCyberpunkSettingRow("16", "Fix Lag (Xóa Đồ Họa)", "Xóa toàn bộ texture, vật liệu giảm lag tối đa.", getgenv().FixLagEnabled, function(state)
...
CreateCyberpunkSettingRow("17", "Lưu Cài Đặt (Save)", "Tự động lưu trạng thái bật/tắt (No Lag).", getgenv().AutoSave, function(state)
    getgenv().AutoSave = state
end)

CreateCyberpunkCycleRow("18", "Ngôn Ngữ (Language)", "Chuyển đổi ngôn ngữ hiển thị UI.", getgenv().LangList, getgenv().LanguageIndex, function(index)
...
local Translations = {
    [1] = { 
        ...
        T6 = "Bật/Tắt Nhạc Nền", D6 = "Nghe nhạc cực chill.",
        T7 = "Âm Lượng Nhạc", D7 = "Điều chỉnh to nhỏ cho phù hợp.",
        T8 = "Chọn Bài Hát", D8 = "Đổi bài hát yêu thích của bạn.",
        T9 = "Nhạc Custom (ID)", D9 = "Dán ID nhạc và nhấn nút ▶ để phát đè lên nhạc có sẵn.",
        T10 = "Auto Farm Killer V1", D10 = "Chờ 3s tàng hình, spam 1 chiêu cơ bản, quét sạch.",
        T11 = "Auto Farm Killer V2", D11 = "Chờ 3s tàng hình, xả 100% Skill VIP, diệt sạch tự Hop.",
        T12 = "Auto Farm Level V1", D12 = "Tự đổi tướng < Lv100 khi con đang xài đã max.",
        T13 = "Auto Hop V1 (Mặc Định)", D13 = "Tìm server ít người, ping ổn định (<150ms).",
        T14 = "Auto Hop V2 (Siêu Mượt)", D14 = "Tìm server ít người và ping thấp khắt khe (<120ms).",
        T15 = "Hop Sau 10 Phút", D15 = "Tự động đổi Server sau mỗi 10 phút chơi.",
        T16 = "Fix Lag (Xóa Đồ Họa)", D16 = "Xóa toàn bộ texture, vật liệu giảm lag tối đa.",
        T17 = "Lưu Cài Đặt (Save)", D17 = "Tự động lưu trạng thái bật/tắt (No Lag).",
        T18 = "Ngôn Ngữ (Language)", D18 = "Chuyển đổi Tiếng Việt / English.",
        Title = "PREMIUM AMETHYST HUB",
        Note = "Bật V2/V3 sẽ tự động tắt cái còn lại chống xung đột."
    },
    [2] = { 
        ...
        T6 = "Toggle BGM", D6 = "Listen to chill music.",
        T7 = "Music Volume", D7 = "Adjust the volume level.",
        ...
        Title = "PREMIUM AMETHYST HUB",
        Note = "Enabling V2/V3 automatically disables the others."
    }
}
...
Notify("Anti-Ban System V4 (Premium Hook) Active!")
                                ...
                                if killer and killer:FindFirstChild("HumanoidRootPart") then
                                    ...
                                    elseif getgenv().AutoEvade_V3 then
                                        ...
                                        if distToKiller < SafeDistance then
                                            ...
                                            Notify("KILLER ĐẾN (<20m)! Bay lên nóc 30m...")
                                            ...
                                            if safeGen then
                                                ...
                                                root.CFrame = CFrame.new(safePos + Vector3.new(0, 30, 0))
                                                root.Velocity = Vector3.zero
                                                root.Anchored = true
                                                SetStatus("Dang tron Killer tren cao (6s)...")
                                                ...
                                                for i = 1, 60 do 
                                                    ...
                                                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                                        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(safePos + Vector3.new(0, 30, 0))
                                                        LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.zero
                                                    end
                                                    task.wait(0.1)
                                                end
